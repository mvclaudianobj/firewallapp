...
function build() {
    ...
    if [[ "$TARGET" == *"-linux-"* ]]; then
        PKG_NAME="firewallapp-${VERSION}-${TARGET}.tar.xz"
        ...
        if [[ "${use_upx}" = true ]]; then
            # Enable upx for MIPS.
            $upx firewallapp #>/dev/null
        fi

        echo "* Packaging XZ in ${PKG_PATH} ..."
        tar -cJf ${PKG_PATH} "firewallapp"
        ...
        PKG_NAME="firewallapp-${VERSION}-${TARGET}.zip"
        PKG_PATH="${PKG_DIR}/${PKG_NAME}"

        echo "* Packaging ZIP in ${PKG_PATH} ..."
        cd ${RELEASE_DIR}
        zip ${PKG_PATH} "firewallapp.exe"
